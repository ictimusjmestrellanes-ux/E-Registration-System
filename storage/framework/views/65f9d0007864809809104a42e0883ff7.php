...
<?php $__env->startSection('content'); ?>
    <?php
        ...
        $fingerprintPreview =
            $editingClient && $editingClient->fingerprint_path
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
                            ...
                            <a href="<?php echo e(route('client.list')); ?>" class="btn btn-sm btn-primary">Clients List</a>
                        </div>
...
unset($__errorArgs, $__bag); ?>
                                    ...
                                    <div class="border rounded-4 p-3 bg-light-subtle">
                                        <div
                                            ...
                                            <div>
                                                ...
                                                <p class="text-muted mb-0 small">Where the client lives and how to reach
                                                    them.</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-check form-switch mb-3">
                                            ...
                                            <label class="form-check-label" for="sameAsHomeAddress">Check if address is
                                                outside City of Imus</label>
                                        </div>
                                ...
                                <div class="col-12">
                                    <div class="border rounded-4 p-3 bg-light-subtle">
                                        ...
                                        <div class="row g-3">
                                                            ...
                                                            <?php $__currentLoopData = $sectorOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sectorOption): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                                <div class="form-check mb-1">
                                                                    <input class="form-check-input sector-checkbox"
                                                                        type="checkbox" value="<?php echo e($sectorOption); ?>"
                                                                        id="sectorOption<?php echo e($loop->index); ?>"
                                                                        <?php echo e(in_array($sectorOption, $selectedSectorsArr) ? 'checked' : ''); ?>>
                                                                    <label class="form-check-label"
                                                                        for="sectorOption<?php echo e($loop->index); ?>">
                                            ...
                                            <div class="col-lg-3">
                                                <label for="positionOrganization" class="form-label">Position /
                                                    Organization</label>
                                ...
                                <div class="col-12">
                                    <div class="d-flex justify-content-left gap-2 mt-2">
                                        <button type="submit"
                                            class="btn btn-sm btn-primary"><?php echo e($editingClient ? 'Update Client' : 'Save Client'); ?></button>
                                    </div>
                                </div>
                            </div>
                        </form>
...
<?php $__env->startSection('script'); ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const fingerprintStartBridgeUrl = <?php echo json_encode(route('fingerprint.start-bridge'), 15, 512) ?>;
            ...
            const syncAgeFromBirthDate = (force = false) => {
            ...
            const loadProvinces = async () => {
                ...
                const filteredProvinces = provinces.filter((province) => calabarzonProvinces.includes(
                    province.name));
            ...
            const loadCities = async (provinceCode, preferredCity = '') => {
                ...
                const response = await fetch(
                    `${apiBase}/provinces/${encodeURIComponent(provinceCode)}/cities.json`);
                if (!response.ok) {
            ...
            const loadBarangays = async (cityCode, preferredBarangay = '') => {
                ...
                const response = await fetch(
                    `${apiBase}/cities/${encodeURIComponent(cityCode)}/barangays.json`);
                if (!response.ok) {
            ...
            const showCameraError = (reason) => {
                cameraUnsupportedMessage.classList.remove('d-none');
                cameraView.classList.add('d-none');
                capturePhotoBtn.disabled = true;
                document.getElementById('cameraUnsupportedReason').textContent = reason;
            };
            ...
            const startCamera = async () => {
                ...
                if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                    showCameraError('This browser does not support camera capture.');
                    return;
                }
                ...
                try {
                    ...
                    const reasons = {
                        NotAllowedError: 'Camera permission was denied. Allow it in the browser address bar, then try again.',
            ...
            saveFingerprintBtn.addEventListener('click', function() {
                (async () => {
                    ...
                    if (!imageData || !imageData.startsWith('data:image/')) {
                        ...
                        showFingerprintModalError(
                            'Please wait for the scanner to capture a fingerprint first.');
                        return;
                    }
            ...
            form.addEventListener('reset', function() {
                setTimeout(() => {
                    ...
                    fingerprintPreview.src = originalFingerprintPreview;
                    fingerprintModalPreview.src = originalFingerprintPreview;
                    fingerprintStatus.textContent = existingFingerprint ?
