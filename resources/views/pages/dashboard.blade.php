                    ...
                    <div class="col-12">
                        <div class="card material-shadow h-100">
                            <div class="card-header d-flex flex-wrap gap-2 align-items-center justify-content-between">
                                <div>
                                    <h5 class="mb-0" id="txTrendTitle">Total Transactions{{ ($txCategory ?? '') !== '' ? ' — ' . $txCategory : '' }}</h5>
                                ...
                                <select class="form-select form-select-sm w-auto" id="txCategorySelect"
                                    ...
                                    @foreach (($txCategoryOptions ?? []) as $option)
                                        <option value="{{ $option }}" {{ ($txCategory ?? '') === $option ? 'selected' : '' }}>
                                            {{ $option }}</option>
                                    @endforeach
                                </select>
                            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (txCanvas) {
                ...
                txChart = new Chart(txCanvas, {
                    ...
                    data: {
                        labels: txLabels,
                        datasets: [{
                            label: @json(($txCategory ?? '') !== '' ? $txCategory . ' Transactions' : 'Transactions'),
                            data: txData,
                            borderColor: '#0ac074',
            ...
            if (txCategorySelect) {
                ...
                txCategorySelect.addEventListener('change', async function() {
                    ...
                    try {
                        const url = new URL('{{ route('dashboard.transaction-trend') }}', window.location.origin);
                        if (value) {
