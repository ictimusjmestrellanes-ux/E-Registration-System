                        ...
                        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
                            <div>
                                ...
                                <p class="text-muted mb-0">
                                    Transaction {{ $transaction->transaction_id }} &mdash;
                                    {{ strtoupper($client->full_name ?? 'Client') }}
                                </p>
                            </div>
                            <div class="d-flex gap-2">
                                <a href="{{ route('transactions.process', $transaction->id) }}"
                                    class="btn btn-outline-primary">View Process</a>
                                <a href="{{ route('clients.show', $client) }}" class="btn btn-secondary">Back to
                                    Client</a>
                            </div>
                        </div>
        ...
        <div class="row gx-3 align-items-center mb-3">
            ...
            <div class="col-auto flex-fill">
                <input type="text" id="clientIdInput"
                    ...
                    value="{{ $client->client_id ?? '' }}" readonly>
            </div>
            ...
            <div class="col-auto flex-fill">
                <input type="text" class="form-control form-control-sm border-0 bg-light fw-bold"
                    value="{{ $isEditMode ? $transaction->transaction_date->format('m/d/Y') : now()->format('m/d/Y') }}"
                    readonly>
        ...
        <div class="row gx-3 align-items-center mb-3">
            ...
            <div class="col">
                <select class="form-select form-select-sm border-0 bg-light text-uppercase" name="category"
                    ...
                    <option value="social_services_assistance" @selected($isEditMode && $transaction->category === 'social_services_assistance')>SOCIAL SERVICES ASSISTANCE
                    </option>
...
<div class="wizard-step d-none" data-step="2">
    ...
    <div class="px-4 py-3">
        <div class="row gx-3 align-items-center mb-3">
            ...
            <div class="col-md-3">
                <input type="text" id="wizTransactionIdDisplay"
                    ...
                    value="{{ $isEditMode ? $transaction->transaction_id : 'AUTO-GENERATED' }}" readonly>
            </div>
        ...
        <div class="row gx-3 align-items-center mb-3">
            ...
            <div class="col-md-9">
                <input type="text" class="form-control form-control-sm border-0 bg-light text-uppercase fw-bold"
                    value="{{ strtoupper($client->full_name ?? '') }}" readonly>
            </div>
        </div>
        ...
        <div class="row gx-3 align-items-center mb-3">
            ...
            <div class="col-md-9">
                <input type="text" class="form-control form-control-sm border-0 bg-light text-uppercase fw-bold"
                    value="{{ strtoupper(collect([$client->address, $client->barangay, $client->city, $client->province])->filter()->implode(', ') ?:'-') }}"
                    readonly>
            </div>
        </div>
        ...
        <div class="row gx-3 align-items-center mb-3">
            ...
            <div class="col-md-3">
                <input type="text" class="form-control form-control-sm border-0 bg-light text-uppercase fw-bold"
                    value="{{ strtoupper(auth()->user()->name ?? '') }}" readonly>
            </div>
            <div class="col-md-3 text-uppercase text-muted small fw-semibold">Signatory</div>
            <div class="col-md-3">
                <input type="text" class="form-control form-control-sm border-0 bg-light text-uppercase fw-bold"
                    name="signatory" value="{{ $isEditMode ? $transaction->signatory : '' }}" readonly>
            </div>
        </div>
        ...
        <div class="row gx-3 align-items-center mb-3">
            ...
            <div class="col-md-3">
                <input type="text" class="form-control form-control-sm text-uppercase"
                    name="personnel_endorsed_to" value="{{ $isEditMode ? $transaction->personnel_endorsed_to : '' }}"
            ...
            <div class="col-md-3">
                <input type="text" class="form-control form-control-sm text-uppercase" name="responsible_office"
                    value="{{ $isEditMode ? $transaction->responsible_office : '' }}" placeholder="Enter office">
            </div>
        </div>
        ...
        <div class="row gx-3 align-items-center mb-3">
            ...
            <div class="col-md-3">
                <input type="number" class="form-control form-control-sm" name="amount" step="0.01"
                    min="0" placeholder="0.00" value="{{ $isEditMode ? $transaction->amount : '' }}">
            </div>
        </div>
        ...
        <div class="row gx-3 gy-3">
            <div class="col-12">
                ...
                <textarea class="form-control form-control-sm text-uppercase" name="description" rows="3"
                    placeholder="Enter description (optional)">{{ $isEditMode ? $transaction->description : '' }}</textarea>
            </div>
            <div class="col-12">
                ...
                <textarea class="form-control form-control-sm text-uppercase" name="actions_taken" rows="3"
                    placeholder="Enter actions taken (optional)">{{ $isEditMode ? $transaction->actions_taken : '' }}</textarea>
            </div>
            <div class="col-12">
                ...
                <textarea class="form-control form-control-sm text-uppercase" name="remarks" rows="3"
                    placeholder="Enter remarks (optional)">{{ $isEditMode ? $transaction->remarks : '' }}</textarea>
            </div>
        </div>
    </div>
</div>
                ...
                <div class="p-3 border rounded-3 bg-light h-100">
                    ...
                    @if ($isEditMode && $wizCur1)
                        <div class="small text-muted mt-2" id="wizReqCurrent1">
                            @if ($wizCur1->file_path)
                                Current file: <a
                                    href="{{ route('transaction-requirements.download', $wizCur1->id) }}">{{ $wizCur1->file_name }}</a>
                            @else
                    ...
                    <div class="form-check mt-2">
                        <input class="form-check-input wizard-no-file" type="checkbox" id="wizReqNoFile1"
                            data-upload-id="wizReqUpload1" @checked($isEditMode && $wizCur1 && !$wizCur1->file_path)>
                ...
                <div class="p-3 border rounded-3 bg-light h-100">
                    ...
                    @if ($isEditMode && $wizCur2)
                        <div class="small text-muted mt-2" id="wizReqCurrent2">
                            @if ($wizCur2->file_path)
                                Current file: <a
                                    href="{{ route('transaction-requirements.download', $wizCur2->id) }}">{{ $wizCur2->file_name }}</a>
                            @else
                                <span class="fst-italic">Currently marked: no file provided</span>
                            @endif
                        </div>
                    ...
                    <div class="form-check mt-2">
                        <input class="form-check-input wizard-no-file" type="checkbox" id="wizReqNoFile2"
                            data-upload-id="wizReqUpload2" @checked($isEditMode && $wizCur2 && !$wizCur2->file_path)>
                ...
                <div class="p-3 border rounded-3 bg-light h-100">
                    ...
                    @if ($isEditMode && $wizCur3)
                        <div class="small text-muted mt-2" id="wizReqCurrent3">
                            @if ($wizCur3->file_path)
                                Current file: <a
                                    href="{{ route('transaction-requirements.download', $wizCur3->id) }}">{{ $wizCur3->file_name }}</a>
                            @else
                                <span class="fst-italic">Currently marked: no file provided</span>
                            @endif
                        </div>
                    ...
                    <div class="form-check mt-2">
                        <input class="form-check-input wizard-no-file" type="checkbox" id="wizReqNoFile3"
                            data-upload-id="wizReqUpload3" @checked($isEditMode && $wizCur3 && !$wizCur3->file_path)>
        ...
        <div class="row g-3 align-items-center">
            ...
            <div class="col-md-9">
                <input type="text" class="form-control form-control-sm text-uppercase" id="wizSubjectFirstName"
                    name="first_name" value="{{ $isEditMode ? $transaction->subject_first_name : '' }}" required>
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-9">
                <input type="text" class="form-control form-control-sm text-uppercase" id="wizSubjectMiddleName"
                    name="middle_name" value="{{ $isEditMode ? $transaction->subject_middle_name : '' }}">
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-9">
                <input type="text" class="form-control form-control-sm text-uppercase" id="wizSubjectLastName"
                    name="last_name" value="{{ $isEditMode ? $transaction->subject_last_name : '' }}" required>
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-3">
                <input type="text" class="form-control form-control-sm text-uppercase" id="wizSubjectNameExt"
                    name="name_ext" value="{{ $isEditMode ? $transaction->subject_name_ext : '' }}">
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-3">
                <input type="date" class="form-control form-control-sm" id="wizSubjectBirthdate" name="birthdate"
                    value="{{ $isEditMode ? optional($transaction->subject_birthdate)->format('Y-m-d') : '' }}"
                    required>
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-4">
                <input type="number" class="form-control form-control-sm bg-warning-subtle" id="wizSubjectAge"
                    ...
                    value="{{ $isEditMode ? $transaction->subject_age : '' }}">
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-9">
                <input type="text" class="form-control form-control-sm text-uppercase" id="wizSubjectBarangay"
                    name="barangay" value="{{ $isEditMode ? $transaction->subject_barangay : '' }}" required>
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-9">
                <input type="text" class="form-control form-control-sm text-uppercase" id="wizSubjectMunicipality"
                    name="municipality" value="{{ $isEditMode ? $transaction->subject_municipality : '' }}" required>
            </div>
            <div class="col-md-3">
        ...
        <div class="modal-content">
            <div class="modal-body text-center py-4">
                ...
                <p class="text-muted mb-0">Are you sure you want to
                    {{ $isEditMode ? 'save the changes to this' : 'submit this' }} transaction?</p>
            </div>
            <div class="modal-footer border-0 justify-content-center gap-3 pt-0">
                ...
                <button type="button" class="btn btn-primary px-4"
                    id="submitTransactionBtn">{{ $isEditMode ? 'Update' : 'Submit' }}</button>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-md">
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    <img id="requirementPreviewImage{{ $num }}" src="" alt="Preview"
                        class="img-fluid rounded d-none" style="max-height: 70vh;" />
                    <iframe id="requirementPreviewFrame{{ $num }}" src="" class="d-none"
