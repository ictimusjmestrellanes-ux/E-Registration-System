...
@foreach ($reqModalLabels as $num => $label)
...
const subjectSaveUrlTemplate = @json(route('transactions.subject.store', ['id' => '__TRANSACTION_ID__']));
...
async function saveRequirements(btn, options = {}) {
    ...
    try {
        ...
        for (const req of requirements) {
            ...
            const response = await fetch('{{ route('transaction-requirements.store') }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '',
                    'Accept': 'application/json'
